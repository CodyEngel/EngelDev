...
class YouTubeVideoRepository(
	...
	suspend fun retrieveVideos(limit: Int = 21): List<YouTubeVideo> {
		...
		return tracer.spanBuilder("RetrieveYouTubeVideos").startScopedSpan().use {
			fetchedAt?.let { date ->
				val now = Date()
	...
	private suspend fun retrieveVideosFromNetwork(limit: Int): List<YouTubeVideo> {
		return tracer.spanBuilder("RetrieveVideosFromNetwork").startScopedSpan().use {
	...
	private suspend fun retrieveVideosFromCache(limit: Int): List<YouTubeVideo> {
		return tracer.spanBuilder("RetrieveVideosFromCache").startScopedSpan().use {
	...
	private fun updateFetchedAt() {
		...
		tracer.spanBuilder("UpdateFetchedAt").startScopedSpan().use {
